...
with open('/tmp/respostasbaseadasemevidnciasparaproblemasematenoprimria.wordpress.2014-10-13.xml') as handle:
	data = handle.read()
	data = BeautifulSoup(data, convertEntities=BeautifulSoup.HTML_ENTITIES)
...
for item in data.findAll('item'):
	...
	post['id'] = item.find('wp:post_id').text
	post['title'] = item.find('title').text
	post['content'] = item.find('content:encoded').text
	post['date'] = item.find('wp:post_date').text
...
for post in posts:
	...
	""" % (post['date'], post['date'], post['title'], post['id'], 'draft', tax, post['content'])
